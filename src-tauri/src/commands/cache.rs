use crate::services::cache::{CacheConfig, CacheManager, CacheStatus, CacheType};
...
pub async fn install_redis(app: AppHandle) -> Result<String, String> {
    ...
    // Download Redis
    let url = CacheManager::get_redis_download_url();
...
/// Install Memcached
#[command]
pub async fn install_memcached(app: AppHandle) -> Result<String, String> {
    let bin_dir = app
        .path()
        .app_local_data_dir()
        .map_err(|e| e.to_string())?
    ...
    if memcached_dir.exists() {
        return Err("Memcached is already installed".to_string());
    }

    // Create directory
    fs::create_dir_all(&memcached_dir)
        .map_err(|e| format!("Failed to create Memcached directory: {}", e))?;

    // Download Memcached
    let url = CacheManager::get_memcached_download_url();
    let zip_path = bin_dir.join("memcached.zip");

    download_file(url, &zip_path).await?;

    // Extract
    extract_zip(&zip_path, &memcached_dir)?;

    // Clean up zip
    let _ = fs::remove_file(&zip_path);

    // Create default config/script
    let config = CacheConfig {
    ...
    CacheManager::create_memcached_config(&app, &config)?;

    Ok("Memcached installed successfully".to_string())
}

/// Uninstall Memcached
#[command]
pub fn uninstall_memcached(app: AppHandle) -> Result<String, String> {
    let bin_dir = app
        .path()
        .app_local_data_dir()
        .map_err(|e| e.to_string())?
    ...
    fs::remove_dir_all(&memcached_dir)
        .map_err(|e| format!("Failed to remove Memcached: {}", e))?;

    Ok("Memcached uninstalled successfully".to_string())
}
...
#[command]
pub fn update_memcached_config(app: AppHandle, port: u16, max_memory: String) -> Result<String, String> {
    let config = CacheConfig {
        cache_type: CacheType::Memcached,
        port,
        max_memory,
    ...
    CacheManager::create_memcached_config(&app, &config)?;
    Ok("Memcached configuration updated".to_string())
}
...
/// Get Memcached executable path for service manager
#[command]
pub fn get_memcached_exe_path(app: AppHandle) -> Result<String, String> {
    let bin_dir = app
        .path()
        .app_local_data_dir()
        .map_err(|e| e.to_string())?
    ...
    if !memcached_exe.exists() {
        return Err("Memcached is not installed".to_string());
    }

    Ok(memcached_exe.to_string_lossy().to_string())
}
