...
export function ServiceManager() {
  ...
  const [available, setAvailable] = useState<{
    nginx: ServiceVersion[];
    apache: ServiceVersion[];
    php: ServiceVersion[];
    mariadb: ServiceVersion[];
    nodejs: ServiceVersion[];
    python: ServiceVersion[];
    bun: ServiceVersion[];
  }>({ nginx: [], apache: [], php: [], mariadb: [], nodejs: [], python: [], bun: [] });
  ...
  const isVersionInstalled = (type: string, version: string) => {
    return services.some(s =>
      s.service_type === type && s.version === version
    );
  };
  ...
  const fetchAvailable = async (forceRefresh = false) => {
    setLoading(true);
    try {
      // Use allSettled so one failure doesn't break all fetches
      const results = await Promise.allSettled([
        getAvailableVersions('nginx', forceRefresh),
        getAvailableVersions('apache', forceRefresh),
        getAvailableVersions('php', forceRefresh),
        getAvailableVersions('mariadb', forceRefresh),
        getAvailableVersions('nodejs', forceRefresh),
        getAvailableVersions('python', forceRefresh),
        getAvailableVersions('bun', forceRefresh)
      ]);

      // Extract successful results, default to empty array on failure
      const getValue = (result: PromiseSettledResult<ServiceVersion[]>) =>
        result.status === 'fulfilled' ? result.value : [];

      setAvailable({
        nginx: getValue(results[0]),
        apache: getValue(results[1]),
        php: getValue(results[2]),
        mariadb: getValue(results[3]),
        nodejs: getValue(results[4]),
        python: getValue(results[5]),
        bun: getValue(results[6])
      });

      // Log any failures
      results.forEach((result, index) => {
        if (result.status === 'rejected') {
          const services = ['nginx', 'apache', 'php', 'mariadb', 'nodejs', 'python', 'bun'];
          console.warn(`Failed to fetch ${services[index]} versions:`, result.reason);
        }
      });
    } catch (e) {
      console.error(e);
            ...
            {!loading && (
              <>
                <ServiceGroup
                  ...
                  icon="🌐"
                  versions={available.nginx}
                  type="nginx"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('nginx', v)}
                />
                <ServiceGroup
                  title="Apache HTTP Server"
                  icon="🪶"
                  versions={available.apache}
                  type="apache"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('apache', v)}
                />
                <ServiceGroup
                  title="PHP Interpreter"
                  icon="🐘"
                  versions={available.php}
                  type="php"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('php', v)}
                />
                <ServiceGroup
                  title="MariaDB Database"
                  icon="🗄️"
                  versions={available.mariadb}
                  type="mariadb"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('mariadb', v)}
                />
                ...
                <ServiceGroup
                  ...
                  icon="💚"
                  versions={available.nodejs}
                  type="nodejs"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('nodejs', v)}
                />
                <ServiceGroup
                  title="Python"
                  icon="🐍"
                  versions={available.python}
                  type="python"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('python', v)}
                />
                <ServiceGroup
                  title="Bun Runtime"
                  icon="🥟"
                  versions={available.bun}
                  type="bun"
                  onInstall={handleInstall}
                  processing={processing}
                  isInstalled={(v) => isVersionInstalled('bun', v)}
                />
              </>
            )}
          </div>
