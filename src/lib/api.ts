...
export const readLogs = async (service: string, logType: string): Promise<string[]> => {
  try {
    return await invoke('read_logs', { service, logType });
  } catch (error) {
...
export const readLogRaw = async (path: string, lines: number): Promise<string[]> => {
  try {
    return await invoke('read_log_raw', { path, lines });
  } catch (error) {
    console.error('Failed to read raw log:', error);
    throw error;
  }
};
...
export const getLogSize = async (path: string): Promise<number> => {
  try {
    return await invoke('get_log_size', { path });
  } catch (error) {
...
// Cache (Redis/Memcached) Types
export interface CacheStatus {
  redis_installed: boolean;
  redis_path: string | null;
  redis_running: boolean;
  redis_port: number;
  memcached_installed: boolean;
  memcached_path: string | null;
  memcached_running: boolean;
...
export const installMemcached = async (): Promise<string> => {
  try {
    return await invoke('install_memcached');
  } catch (error) {
    console.error('Failed to install Memcached:', error);
    throw error;
  }
};

export const uninstallMemcached = async (): Promise<string> => {
  try {
    return await invoke('uninstall_memcached');
  } catch (error) {
    console.error('Failed to uninstall Memcached:', error);
    throw error;
  }
};
...
export const updateMemcachedConfig = async (port: number, maxMemory: string): Promise<string> => {
  try {
    return await invoke('update_memcached_config', { port, maxMemory });
  } catch (error) {
    console.error('Failed to update Memcached config:', error);
    throw error;
  }
};
...
export const getMemcachedExePath = async (): Promise<string> => {
  try {
    return await invoke('get_memcached_exe_path');
  } catch (error) {
    console.error('Failed to get Memcached exe path:', error);
    throw error;
  }
};
