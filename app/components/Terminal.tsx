...
export function Terminal({ className, onClose }: TerminalProps) {
  ...
  // Watch for theme changes and update existing terminals
  useEffect(() => {
    const themeToApply =
      resolvedTheme === "light" ? XTERM_THEME_LIGHT : XTERM_THEME_DARK;
    xtermsRef.current.forEach(({ term }) => {
      term.options.theme = themeToApply;
    });
  }, [resolvedTheme]);

  // Keep a ref for initXterm without adding to deps
  ...
  const initXterm = useCallback(
    async (tabId: string, cwd?: string, initialCommand?: string) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      try {
        entry.fitAddon.fit();
        resizeTerminal(activeTabId, entry.term.cols, entry.term.rows).catch(
          console.error,
        );
      } catch {
        /* fit may fail if terminal not visible */
      }
    }, 50);
  ...
  const manageableServices = services.filter((s) =>
    MANAGEABLE_TYPES.includes(s.service_type),
  );
        ...
        <div className="flex-1 min-h-0 relative">
          {tabs.length === 0 && (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-content-muted gap-3">
              ...
              <p className="text-sm">
                Click <span className="text-emerald-500 font-medium">+</span> or
                a site to open a terminal
              </p>
            </div>
          )}
          {tabs.map((tab) => (
