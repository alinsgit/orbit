...
pub async fn download_file(url: &str, dest_path: &PathBuf) -> Result<(), String> {
    // Create client with proper User-Agent to avoid being blocked
    ...
    if let Some(content_type) = res.headers().get("content-type") {
        ...
        if ct.contains("text/html") {
            return Err("Download blocked: received HTML instead of file (possibly CloudFlare protection)".to_string());
        }
    }
    ...
    if total_size > 0 && downloaded != total_size {
        return Err(format!("Download incomplete: {} of {} bytes", downloaded, total_size));
    }

    println!("Download complete: {} bytes written to {:?}", downloaded, dest_path);
    Ok(())
}
