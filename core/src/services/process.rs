...
impl ServiceManager {
    ...
    pub fn start_with_name(
        ...
        if let Some(port) = get_service_port(&name) {
            if is_port_in_use(port) {
                if Self::is_orbit_process_running(&name) {
                    log::info!("Service {name} already running on port {port}");
                    return Ok(0); // Orbit's own service, report success
                } else {
                    return Err(format!(
                        "Port {port} is already in use by another application (not Orbit). \
                         Stop the conflicting process before starting {name}."
                    ));
                }
            }
        }
        ...
        match command.spawn() {
            Ok(mut child) => {
                ...
                if is_runtime {
                    ...
                    // Poll briefly to verify the process didn't crash immediately
                    // Check every 100ms up to 1.5s — returns as soon as we're confident
                    for i in 0..15 {
                        ...
                        match child.try_wait() {
                            ...
                            Ok(None) => {
                                // Still running — after 500ms we're confident enough
                                if i >= 4 {
                                    let mut processes =
                            ...
                            Err(e) => return Err(format!("Failed to check service status: {e}")),
                        }
                    }
                    // Reached max iterations, still running — success
                    let mut processes = self.processes.lock().map_err(|e| e.to_string())?;
                    processes.insert(name, child);
                    Ok(pid)
                }
    ...
    pub fn stop(&self, service_name: &str) -> Result<(), String> {
        ...
        if let Some(mut child) = processes.remove(service_name) {
            ...
            // Not in HashMap — find and kill orphaned Orbit processes by checking bin dir
            let process_names = get_process_names(service_name);
            if process_names.is_empty() {
                return Err("Service not found or not running".to_string());
            }
            ...
            for process in sys.processes().values() {
                ...
                // Only kill processes from Orbit's bin directory
                ...
                let matches = process_names.iter().any(|&n| {
                    let clean_n: String = n.chars()
                        .filter(|c| c.is_alphanumeric()).collect::<String>().to_lowercase();
